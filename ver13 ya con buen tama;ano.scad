...
wallThickness = 4.5; // custom thickness for the added walls

// Helper: hollow box
module hollow_box(sizeX, sizeY, sizeZ, wall) {
    
        cube([sizeX - 2*wall, wall, sizeZ], center = true);
...
// Glued hollow box for top long side
translate([ 0, (sizeY/2), - (sizeZ/2 + extraBoxHeight/2) ])
    hollow_box(sizeX, extraDepth, extraBoxHeight, wallThickness);

// ---- Bottom long side ----
translate([ 0, -(sizeY/2) - (extraDepth/2), 0 ])
    cube([ sizeX, extraDepth, sizeZ ], center = true);

// Glued hollow box for bottom long side
translate([ 0, -(sizeY/2), - (sizeZ/2 + extraBoxHeight/2) ])
    hollow_box(sizeX, extraDepth, extraBoxHeight, wallThickness);
...
// Glued hollow box for right short side
translate([ (sizeX/2), 0, - (sizeZ/2 + extraBoxHeight/2) ])
    hollow_box(extraDepth, sizeY, extraBoxHeight, wallThickness);

// ---- Left short side ----
translate([ -(sizeX/2) - (extraDepth/2), 0, 0 ])
    cube([ extraDepth, sizeY, sizeZ ], center = true);

// Glued hollow box for left short side
translate([ -(sizeX/2), 0, - (sizeZ/2 + extraBoxHeight/2) ])
    hollow_box(extraDepth, sizeY, extraBoxHeight, wallThickness);
